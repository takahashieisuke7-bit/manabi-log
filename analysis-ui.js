...
function renderStudyBars(){
  ...
  const buckets=StudyAnalysis.buckets(records,studyBarsMode,localDateKey(),studyBarsShift),colors=studySubjectColors();
  document.getElementById('studyBarsRange').textContent=`${buckets[0].start} 〜 ${buckets.at(-1).end}（${studyBarsMode==='day'?'日別・7日間':studyBarsMode==='week'?'週別・月曜始まり・8週間':'月別・12か月'}）`;
  ...
  host.replaceChildren();const max=Math.max(60,Math.ceil(Math.max(...buckets.map(b=>b.total))/60)*60);
  ...
  for(const bucket of buckets){
    ...
    column.append(stack);button.append(chartNode('span','study-bar-total',String(bucket.total)),column,chartNode('span','study-bar-label',bucket.label));
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  for(const [id,delta] of [['studyBarsPrevious',-1],['studyBarsNext',1],['studyBarsToday',0]])document.getElementById(id).addEventListener('click',()=>{studyBarsShift=delta?studyBarsShift+delta:0;studyBarsSelection='';renderStudyBars();});
});
